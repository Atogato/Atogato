...
export default function Filteredlist({ options }: optionsProps): JSX.Element {
  ...
  const filterbyLocation = projects.filter((project) => project.location === options[0])
  const filterbyCategory = projects.filter((project) => project.requiredCategory === options[1])
