...
export default function Timeline(){
    ...
    return(
        <div className="grid grid-cols-7 gap-4">        
            {thisWeek.map((item, i)=>(
                <div className={`col-start-${i + 1} col-end-${i + 2} flex justify-center ${item.day === thisWeek[0].day ? 'bg-orange-200' : 'bg-teal-200'}`} key={i}>
            ...
            {projects.map((project,i)=>{
                ...
                let d3 
                  {(dl >= thisWeek[0].day)? 
                    d3 = dl-thisWeek[0].day
                    :
                    d3 = 7- thisWeek[0].day + dl
                  }
                  console.log(d3)
                return (
